...
│
├── run_pipeline.py               # Runs all notebooks sequentially
├── download_nltk_data.py         # One-time NLTK download
├── COLAB_SETUP.md                # Google Colab setup guide (GPU acceleration) - recommended
│
│
├── report/
│   └── report.pdf                # Final written report (git-ignored)
│
├── presentation/
│   └── slides.pdf                # Presentation slides (git-ignored)
│
└── reflections/                  # Reflection notes (git-ignored)
    ├── ls_reflection.pdf
    └── cws_reflection.pdf
```
...
| Random Forest (TF-IDF) | ~85.7% | ~0.857 | Ensemble method |
